...
export const GuildSelector: FC<GuildSelectorProps> = ({ wallet }) => {
  ...
  const memberRank = guild ? guildService.getGuildMembers(guild.id).findIndex(m => m.wallet === wallet) + 1 : null;
  const totalMembers = guild ? guildService.getGuildMembers(guild.id).length : 0;
              ...
              <div className="flex items-center justify-center gap-2 mb-4">
                <div className="flex items-center gap-1.5 px-3 py-1 bg-primary-pink/10 rounded-full">
                  ...
                  <span className="text-sm text-primary-pink font-medium">
                    Guild Member #{memberRank} of {totalMembers}
                  </span>
                </div>
              </div>
